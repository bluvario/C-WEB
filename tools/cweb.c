...
static void emit_main(FILE *f, const Str_List *views, const char *static_root, int has_404)
{
    fputs(
        ...
        "// meaningful under a real server\n"
    ...
    for (size_t i = 0; i < views->count; i++) {
        ...
        if (strcmp(route, "/404") == 0 || is_partial(views->items[i])) {
            continue; // not routes: /404 becomes the fallback, partials never route
        }
    ...
    for (size_t i = 0; i < views->count; i++) {
        ...
        route_of(views->items[i], route, sizeof route);
        if (strcmp(route, "/404") == 0 || is_partial(views->items[i])) {
            continue; // partials are fragments; /404 is the not-found fallback
        }
    ...
    fputs(
        ...
        "    int rc;\n"
        "    if (rate_per_min > 0) {\n"
        "        // N requests a minute per client: the bucket starts full so a\n"
        "        // momentary burst of N passes, beyond that each client is\n"
        "        // answered with 429 until the bucket refills\n"
        ...
        "        http_middleware_add(&chain, http_rate_limit_middleware, &limiter);\n"
...
static int cmd_build(int argc, char **argv)
{
    ...
    emit_main(f, &views, static_arg, has_404);
    fclose(f);
...
static void usage(FILE *f)
{
    fprintf(f,
        ...
        "The built OUT_DIR/server accepts --port N and --rate N, the latter\n"
        "capping each client at N requests per minute.\n");
}
