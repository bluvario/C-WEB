...
{
    fprintf(f,
        "usage: cweb <command> [options]\n"
        "\n"
        "  build VIEWS_DIR OUT_DIR [ROOT [STATIC]]\n"
        "      compile every VIEWS_DIR/**/*.c.html into C, write OUT_DIR/main.c\n"
        "      and link OUT_DIR/server against the framework\n"
        ...
        "build/libcweb.a; when the path given does not hold an include/ dir the\n"
        "tool walks up to the nearest ancestor that does, so \"cweb serve views\n"
        "8080 . static\" also works from inside an app directory. STATIC, when\n"
        "given, is a directory mounted at /* and served for any path a page\n"
        "does not claim.\n"
        "\n"
        "A views/404.c.html page becomes the app's custom not-found page, served\n"
        "with a 404 status for any path nothing else answers. Files under\n"
        "VIEWS_DIR/partials/ become reusable fragments: compiled and declared\n"
        "in OUT_DIR/pages.h but never routed, so a view can render one with\n"
        "<?c page_footer(req, res, params, user_data); ?>. A views/layout.c.html\n"
        "is the wrapper convention: compiled but never routed, it embeds the\n"
        "captured page body with <?c cweb_tpl_layout_emit(res); ?> and so shapes\n"
        "every page (including the 404 page).\n"
        "\n"
        "The built OUT_DIR/server accepts these flags:\n"
        "  --port N                        listen port (default 8080)\n"
        "  --rate N                        a per-minute budget of N requests per\n"
        "                                  caller: their address unless the\n"
        "                                  request names an account (Basic creds\n"
        "                                  map to that account even while the\n"
        "                                  password is still being checked, so a\n"
        "                                  brute force is throttled across IPs);\n"
        "                                  429 past the budget\n"
        "  --secure                         hardening headers on every response\n"
        "                                  (nosniff, no-referrer, frame-deny,\n"
        "                                  default-src 'self' CSP)\n"
        "  --gzip                           compress compressible bodies\n"
        "  --db PATH                        file-backed key-value store pages\n"
        "                                  reach through cweb_database()\n"
        "  --static-cache SECONDS           Cache-Control max-age on static\n"
        "                                  files (ETag and Last-Modified already\n"
        "                                  answer 304)\n"
        "  --etag                           strong ETag from a SHA-256 of every\n"
        "                                  dynamic response body; a matching\n"
        "                                  If-None-Match returns 304\n"
        "  --request-id                     stamp every response (and every\n"
        "                                  req->request_id) with a unique\n"
        "                                  16-hex trace id\n"
        "  --request-id-inherit             also echo an inbound X-Request-Id\n"
        "                                  from a trusted proxy\n"
        "  --basic-auth USER:PASS           lock the app behind HTTP Basic auth;\n"
        "                                  anything else gets 401 + WWW-Authenticate\n"
        "  --trusted-proxy CIDR[,CIDR...]   believe X-Forwarded-For only from\n"
        "                                  reverse proxies in these networks, so\n"
        "                                  logs and per-address rate buckets name\n"
        "                                  the real client\n"
        "  --max-body BYTES                 request size cap, 413 past it; accepts\n"
        "                                  k/m/g suffixes like 4k or 2m\n"
        "  --body-dir DIR                   with --max-body set, stream oversized\n"
        "                                  bodies to temp files in DIR, mapped back\n"
        "                                  then unlinked, instead of 413\n"
        "  --max-inflated BYTES             ceiling a Content-Encoding: gzip\n"
        "                                  request body may decompress to, 413 past\n"
        "                                  it (default 128m)\n"
        "  --tls-cert FILE --tls-key FILE   PEM chain cert and private key; both\n"
        "                                  turn the whole listener into HTTPS\n"
        "  --io-timeout MS                  per-read deadline; stalled clients get 408\n"
        "  --workers N                      accept-loop threads (default one per core)\n"
        "  --signature-secret SECRET        only trust a reverse proxy that stamps\n"
        "                                  every request with HMAC-SHA256 of\n"
        "                                  \"METHOD\\nPATH\\nUNIX-SECONDS\\nBODY\" in\n"
        "                                  X-CWEB-Signature/X-CWEB-Date; else 403\n"
        "  --socket PATH                    also serve the same app on a unix socket\n"
        "                                  file (removed on shutdown), for local\n"
        "                                  reverse-proxy setups\n"
        "  --cors ORIGIN                    answer CORS preflights and stamp\n"
        "                                  Access-Control-Allow-Origin (\"*\" opens\n"
        "                                  the app to any origin without credentials)\n"
        "  --csp POLICY                     Content-Security-Policy on every response,\n"
        "                                  replacing the built-in default-src 'self'\n"
        "  --csp-report-only POLICY         a monitoring-only policy alongside it\n"
        "  --log FILE                       append Common Log Format access lines\n"
        "  --route-timeout PATH MS          set SO_RCVTIMEO to MS for the next request\n"
